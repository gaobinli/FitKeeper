...
import OrderDetailOperateModal, { type OperateType } from './modules/orderDetail-operate-modal.vue';
import OrderDetailSearch from './modules/orderDetail-search.vue';
...
async function handleBatchDelete() {
  const { error, data } =  await fetchDeleteOrderDetail(checkedRowKeys.value);
  // request
...
async function handleDelete(id: number) {
  // request
  const { error, data } =  await fetchDeleteOrderDetail(id);
  console.log(id);
  onDeleted();
}
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden lt-sm:overflow-auto">
    <OrderDetailSearch v-model:model="searchParams" @reset="resetSearchParams" @search="getDataByPage" />
