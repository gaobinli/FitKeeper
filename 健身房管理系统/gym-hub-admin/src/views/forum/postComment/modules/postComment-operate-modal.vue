...
import {
fetchPostPostComment,
fetchPatchPostComment,
fetchPostList,
fetchUserList,
} from '@/service/api';
...
const rules: Record<RuleKey, App.Global.FormRule> = {
  ...
  'commentContent' : defaultRequiredRule,
...
async function handleSubmit() {
  ...
  if (model.value.id) {
    const { error, data } = await fetchPatchPostComment(model.value.id, model.value);
    if (error) {
      return
    }
  } else {
    const { error, data } = await fetchPostPostComment(model.value);
    if (error) {
      return
    }
  }
...
const getWangEditorValue = (str: string) => {
  model.value.commentContent = str
};
    ...
    <NScrollbar class="h-480px pr-20px">
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
        <NGrid responsive="screen" item-responsive>
          <NFormItemGi span="24 m:24" label="帖子：" path="postId" class="pr-24px">
            <NSelect
          ...
          <NFormItemGi span="24 m:24" label="评论人：" path="userId" class="pr-24px">
            <NSelect
          ...
          <NFormItemGi span="24 m:24" label="评论内容：" path="commentContent">
            <wang-editor :contentHtml="model.commentContent" v-on:getWangEditorValue="getWangEditorValue"></wang-editor>
          </NFormItemGi>
        </NGrid>
      </NForm>
    </NScrollbar>
