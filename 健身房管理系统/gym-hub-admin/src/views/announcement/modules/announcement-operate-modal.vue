...
fetchPatchAnnouncement,
} from '@/service/api';
import { useAuthStore } from '@/store/modules/auth';

defineOptions({
  name: 'AnnouncementOperateModal'
});
...
async function handleSubmit() {
  ...
  if (model.value.id) {
    const { error, data } = await fetchPatchAnnouncement(model.value.id, model.value);
    if (error) {
      return
    }
  } else {
    const { error, data } = await fetchPostAnnouncement(model.value);
    if (error) {
      return
    }
  }
...
// Computed property to filter type options based on user role
const typeOptions = computed(() => {
  ...
  // If user is coach (other role), only show 健身资讯
    ...
    <NScrollbar class="h-480px pr-20px">
      <NForm ref="formRef" :model="model" :rules="rules" label-placement="left" :label-width="100">
        <NGrid responsive="screen" item-responsive>
          ...
          <NFormItemGi span="24 m:24" label="类型：" path="type" class="pr-24px">
            <NSelect
