...
public class SysUserServiceImpl implements SysUserService {
    ...
    public SysUserResDto save(SysUserReqDto reqDto) {
        String userCode = reqDto.getUsername();
    ...
    @Override
    public List<SysUserResDto> getList() {
        List<SysUserPo> list = sysUserPoService.list(Wrappers.<SysUserPo>lambdaQuery().ne(SysUserPo::getId, 1));
        return sysUserResDtoConvertor.poList2DtoList(list);
    }

    public void checkCodeRepeat(String code) {
        List<SysUserPo> list = sysUserPoService.getList(new MapUtil().put("qp-username-eq", code), SysUserPo.class);
    ...
    public SysUserResDto info() {
        ...
        return getInfo(sysUser.getUsername());
    }
...
//        for (SysUserResDto sysUserResDto : resDtoPager.getList()) {
//            sysUserResDto.setPassword(null);
//        }
        return resDtoPager;
    }
    ...
    public void dealUserRoleInfo(List<SysUserResDto> sysUserResDtos) {
        if (!CollectionUtils.isEmpty(sysUserResDtos)) {
            ...
            sysUserResDtos.stream().forEach(res -> {
                List<SysUserRolePo> userRolePoList = userRoleMap.get(res.getUsername());
                ...
                userRolePoList.stream().forEach(userRole -> {
                    ...
                    roleCodeList.add(sysRole.getCode());
                });
    ...
    public SysUserResDto getInfo(String code) {
        SysUserPo sysUserPo = sysUserPoService.getOne(Wrappers.<SysUserPo>lambdaQuery().eq(SysUserPo::getUsername, code));
